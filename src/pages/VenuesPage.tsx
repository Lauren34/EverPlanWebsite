...
export default function VenuesPage({ selectedDate }: VenuesPageProps) {
  const [venues, setVenues] = React.useState<Venue[]>([]); // Dynamic venues state
  const [selectedVenue, setSelectedVenue] = React.useState<Venue | null>(null); // Selected venue state

  // Fetch available venues for the selected date
  const fetchAvailableVenues = async (date: string) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/available-venues?date=${date}`);
      console.log("API Response:", response.data);  // Log the response to check data
      const availableVenues = response.data;

      if (availableVenues.length === 0) {
        console.log("No venues available for the selected date.");
      }

      const formattedVenues = availableVenues.map((venue: any) => ({
        title: venue.title, // Ensure the response contains this field
        subtitle: venue.subtitle, // Ensure the response contains this field
        description: venue.description || "No description available", // Fallback if no description
      }));
  ...
  // Fetch available venues when the selected date changes
  React.useEffect(() => {
    if (selectedDate) {
